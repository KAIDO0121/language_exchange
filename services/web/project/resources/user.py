...
class UserRegister(Resource):
    ...
    def post(cls):
        ...
        try:  # handling race condition
            exist = user.find_by_username(user_json['username'])
            if exist:
                return {"message": 'Username already exists.', "errorCode": 1}, 200
            b = user.save_to_db()
...
class EditProfile(Resource):
    ...
    def put(cls):
        ...
        if user:
            ...
            user.update_user_langs(get_jwt_identity(
            ), user_json["user_offer_langs"], user_json["user_acpt_langs"])
